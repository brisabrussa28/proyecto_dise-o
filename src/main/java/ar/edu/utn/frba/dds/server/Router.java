...
public class Router {
  ...
  public void configure(Javalin app, ObjectMapper mapper) {
    ...
    // Colecciones
    app.get("/admin/colecciones", context -> adminController.listarColecciones(context, modeloConSesion(context)));
  ...
  private List<Estadistica> filtrar(List<Estadistica> lista, String tipo) {
    return lista.stream().filter(e -> tipo.equals(e.getTipo())).toList();
  }
}