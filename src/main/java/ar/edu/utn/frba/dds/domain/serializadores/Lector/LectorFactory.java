...
public class LectorFactory {
  ...
  public <T> Lector<T> create(JsonNode importadorNode, Class<T> clazz) {
    String formato = importadorNode.path("formato").asText().toUpperCase();
    switch (formato) {
      case "JSON":
      ...
      case "CSV":
        char separador = importadorNode.path("separador").asText(",").charAt(0);
  ...
  private <T> FilaConverter<T> createFilaConverter(JsonNode importadorNode, Class<T> clazz) {
    if (clazz.equals(Hecho.class)) {
      String dateFormat = importadorNode.path("formatoFecha").asText("yyyy-MM-dd HH:mm:ss");
      JsonNode mapeoNode = importadorNode.path("mapeoColumnas");

      if (mapeoNode.isMissingNode() || !mapeoNode.isObject()) {
        throw new IllegalArgumentException("La configuración 'mapeoColumnas' es requerida para el importador CSV de Hecho.");
      }
      ...
      // El cast es seguro porque está dentro de la validación de la clase
      @SuppressWarnings("unchecked")
      FilaConverter<T> converter = (FilaConverter<T>) new HechoFilaConverter(dateFormat, mapeoColumnas);
      return converter;
    }
