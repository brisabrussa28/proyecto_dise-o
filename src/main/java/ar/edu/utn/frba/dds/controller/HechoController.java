...
public class HechoController {
  ...
  public List<Hecho> findAll() {
    // findAll sigue filtrando eliminados por defecto
    return HechoRepository.instance().findAll();
  }

  public Long countAll() {
    // countAll sigue filtrando eliminados por defecto
    return HechoRepository.instance().countAll();
  }

  public Hecho modificarHecho(Hecho hechoOriginal, Hecho hechoModificado) {
    if (hechoModificado.getTitulo() != null && !hechoModificado.getTitulo().isBlank()) {
      hechoOriginal.setTitulo(hechoModificado.getTitulo());
    }
    if (hechoModificado.getDescripcion() != null && !hechoModificado.getDescripcion().isBlank()) {
      hechoOriginal.setDescripcion(hechoModificado.getDescripcion());
    }
    if (hechoModificado.getEtiquetas() != null && !hechoModificado.getEtiquetas().isEmpty()) {
    ...
    if (hechoModificado.getCategoria() != null && !hechoModificado.getCategoria().isBlank()) {
      hechoOriginal.setCategoria(hechoModificado.getCategoria());
    }
    if (hechoModificado.getDireccion() != null && !hechoModificado.getDireccion().isBlank()) {
    ...
    if (hechoModificado.getEstado() != null && !hechoModificado.getEstado().toString().isBlank()) {
  ...
  public void editarHecho(Context ctx, Map<String, Object> model) {
    try {
      ...
      if (hecho.getFechaSuceso() != null) {
        String fechaFormateada = hecho.getFechaSuceso().truncatedTo(ChronoUnit.MINUTES).toString();
      ...
      if (hecho.getEtiquetas() != null) {
        String etiquetasStr = String.join(", ", hecho.getEtiquetas().stream().map(e -> e.getNombre()).toList());
        model.put("etiquetasTexto", etiquetasStr);
      }

      if (ctx.pathParamMap().containsKey("fuenteId")) {
        try {
  ...
  public void actualizarHecho(Context ctx) {
    try {
      ...
      if (hechoOriginal.getFechacarga().isBefore(LocalDateTime.now().minusWeeks(1))) {
      ...
      for (UploadedFile archivo : archivos) {
        if (archivo.size() > 0) {
          try {
            ...
            Multimedia nuevaFoto = new Multimedia(archivo.filename(), archivo.contentType(), bytesImagen);
  ...
  public void verHecho(Context ctx, Map<String, Object> model) {
    ...
    if (hecho.getAutor() != null && usuarioId != null) {
      esEditable = hecho.getAutor().getId().equals(usuarioId) && hecho.getFechacarga().isAfter(LocalDateTime.now().minusWeeks(1));
    }
  ...
  public void crearHecho(Context ctx) {
    try {
      ...
      LocalDateTime fechaSuceso = fechaSucesoStr != null && !fechaSucesoStr.isBlank()
                                  ? LocalDateTime.parse(fechaSucesoStr) : LocalDateTime.now();
      ...
      for (UploadedFile archivo : archivos) {
        if (archivo != null && archivo.size() > 0) {
          byte[] bytesImagen = archivo.content().readAllBytes();
          Multimedia foto = new Multimedia(archivo.filename(), archivo.contentType(), bytesImagen);
          fotos.add(foto);
        }
      }
      nuevoHecho.setFotos(fotos);

      nuevoHecho.setAutor(UserRepository.instance().findById(usuarioId));
      validarHecho(nuevoHecho);

      this.subirHecho(nuevoHecho);


      Fuente fuente = fuenteController.findFuenteGlobal();
  ...
  public void getMultimedia(Context ctx) {
    int index = ctx.pathParamAsClass("indice", Integer.class).get();
    Long id = ctx.pathParamAsClass("id", Long.class).get();

    Multimedia item = HechoRepository.instance().findById(id).getFotos().get(index);
  ...
  private Map<String, Object> mapHechoToFrontend(Hecho h) {
    ...
    map.put("hecho_estado", h.getEstado());
    ...
    if (h.getEtiquetas() != null) {
      List<Map<String, String>> etiquetasList = h.getEtiquetas().stream().map(e -> {
    ...
    if (h.getColecciones() != null && !h.getColecciones().isEmpty()) {
      List<Map<String, Object>> coleccionesList = h.getColecciones().stream().map(c -> {
  ...
  public void findAndDelete(Long idHecho) {
    HechoRepository.instance().findAndDelete(idHecho);
  }
}